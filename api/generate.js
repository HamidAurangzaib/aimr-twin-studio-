...
// Public Firebase Web API key for this project (same one shipped in the client).
// Used only to validate that an ID token was issued by THIS Firebase project.
const FIREBASE_WEB_API_KEY =
  process.env.FIREBASE_WEB_API_KEY || "AIzaSyBFqaDpdrUZUDil2YmsdK-dGu_echBRSoE";

/**
 * Verifies a Firebase ID token via Google's Identity Toolkit.
 * Returns the user's uid if valid, otherwise null.
 * Because the API key is project-scoped, a token from any other Firebase
 * project will fail here — so this gates the endpoint to our own users only.
 */
...
export default async function handler(req, res) {
  ...
  // --- Authentication: only logged-in users of this app may generate ---
